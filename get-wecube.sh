...
INSTALLER_URL="https://github.com/degalaxy/delivery-by-terraform/archive/master.zip"
...
if [ "$USE_MIRROR_IN_MAINLAND_CHINA" == "true" ]; then
  ...
  INSTALLER_URL="https://gitee.com/degalaxy/delivery-by-terraform/repository/archive/master.zip"
  INSTALLER_SOURCE_CODE_DIR="$WECUBE_HOME/delivery-by-terraform-master/installer"
fi
...
./invoke-installer.sh "$PROVISIONING_ENV_FILE" wecube-user mysql-docker minio-docker open-monitor-agent
